...
TEST_GROUP(trace)
{
  void setup()
  {
    mbed_client_trace_init(0);
    mbed_client_trace_config_set(g_trace, TRACE_MODE_PLAIN|TRACE_ACTIVE_LEVEL_ALL);
    mbed_client_trace_print_function_set( g_trace, myprint ); 
  }
  void teardown()
  {
    mbed_client_trace_free(g_trace);
  }
};

TEST(trace, version)
{
    STRCMP_EQUAL("0.1.0", mbed_client_trace_get_version(g_trace) );
}
...
TEST(trace, Array)
{
  ...
  mbed_tracef(g_trace, TRACE_LEVEL_DEBUG, "mygr", "%s", mbed_trace_array(longStr, 1000) );
}
...
TEST(trace, LongString)
{
  ...
  mbed_tracef(g_trace, TRACE_LEVEL_DEBUG, "mygr", longStr );
}
...
TEST(trace, TooLong)
{
  ...
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_ALL);
  mbed_tracef(g_trace, TRACE_LEVEL_DEBUG, "mygr", longStr );
...
TEST(trace, ipv6)
{
    ...
    int prefix_len = 64;
    char *str = mbed_trace_ipv6_prefix(g_trace, prefix, prefix_len);
...
TEST(trace, active_level_all_ipv6)
{
  ...
  mbed_tracef(g_trace, TRACE_LEVEL_DEBUG, "mygr", "my addr: %s", mbed_trace_ipv6(g_trace, arr));
...
{
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_DEBUG);
  mbed_tracef(g_trace, TRACE_LEVEL_DEBUG, "mygr", "hep");
  STRCMP_EQUAL("[DBG ][mygr]: hep", buf);
  
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_DEBUG|TRACE_MODE_PLAIN);
  mbed_tracef(g_trace, TRACE_LEVEL_DEBUG, "mygr", "hep");
  STRCMP_EQUAL("hep", buf);
  
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_DEBUG|TRACE_MODE_COLOR);
  mbed_tracef(g_trace, TRACE_LEVEL_ERROR, "mygr", "hep");
...
TEST(trace, active_level_debug)
{
  ...
  mbed_tracef(g_trace, TRACE_LEVEL_INFO, "mygr", "test");
  STRCMP_EQUAL("[INFO][mygr]: test", buf);
  
  mbed_tracef(g_trace, TRACE_LEVEL_WARN, "mygr", "hups");
  STRCMP_EQUAL("[WARN][mygr]: hups", buf);
  
  mbed_tracef(g_trace, TRACE_LEVEL_ERROR, "mygr", "o'ou");
...
{
  buf[0] = 0;
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_INFO);
  
  mbed_tracef(g_trace, TRACE_LEVEL_DEBUG, "mygr", "hep");
  STRCMP_EQUAL("", mbed_trace_last(g_trace));
...
{
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_WARN);
...
{
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_ERROR);
...
{
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_NONE);
...
TEST(trace, active_level_all_1)
{
  ...
  mbed_tracef(g_trace, TRACE_LEVEL_DEBUG, "mygu", "hep");
...
{
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_ALL);
  mbed_client_trace_exclude_filters_set(g_trace, "mygr,mygu");
...
{
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_ALL);
  mbed_client_trace_include_filters_set(g_trace, "mygr");
...
TEST(trace, active_level_all_array)
{
  ...
  mbed_tracef(g_trace, TRACE_LEVEL_DEBUG, "mygr", "my addr: %s", mbed_trace_array(g_trace, arr, 3));
...
{
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_ALL);
  mbed_client_trace_prefix_function_set(g_trace,  &trace_prefix );
  mbed_tracef(g_trace, TRACE_LEVEL_DEBUG, "mygr", "test");
  STRCMP_EQUAL("[<TIME>][DBG ][mygr]: test", buf);
  //TEST_ASSERT_EQUAL_INT(4, time_length);
  
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_ALL|TRACE_MODE_PLAIN);
  mbed_tracef(g_trace, TRACE_LEVEL_DEBUG, "mygr", "test");
  STRCMP_EQUAL("test", buf);
  
  mbed_client_trace_config_set(g_trace, TRACE_ACTIVE_LEVEL_ALL|TRACE_MODE_COLOR);
  mbed_tracef(g_trace, TRACE_LEVEL_ERROR, "mygr", "test");
